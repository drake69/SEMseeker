# ---------------------------------------------------------------------------
# read_pivot() — unified dispatcher for marker pivot access
#
...
#' @keywords internal
#' @noRd
read_pivot <- function(marker, figure, area = "POSITION", subarea = "WHOLE",
                       cache = TRUE) {
  ...
  if (length(bed_files) > 0) {
    ...
    lazy <- stream_merge_bed(bed_files, marker, figure)
...
#' @keywords internal
#' @noRd
list_bed_files_for_marker_figure <- function(marker, figure) {
  ssEnv <- get_session_info()
