assoc_pow10_model <- function (family_test, tempDataFrame, sig.formula, transformation_y, plot, samples_sql_condition=samples_sql_condition,key)
{
  ...
  for (cov in cov_temp) {
    ...
    cov_temp[cov_temp==cov] <- paste0("assoc_pow(10,", cov, ")")
  }
  ### changes

  # Build dynamic formula for lm (to get starting values)
  if (length(covariates) > 0)
    lm_formula_str <- paste0(dependent_variable, " ~ assoc_pow(10,", independent_variable, ") + ", paste(cov_temp, collapse = " + "))
  else
    lm_formula_str <- paste0(dependent_variable, " ~ assoc_pow(10,", independent_variable, ")")
  ...
  if(length(covariates) == 0)
    rhs <- paste0("a + b * assoc_pow(10,", independent_variable, ")")
  else
    rhs <- paste0("a +  assoc_pow(10,b * ", independent_variable, ")",paste(paste0(" + assoc_pow(10,", cov_letters[2:length(cov_letters)], " * ", covariates, ")"), collapse = ""))
  ...
  if(plot & !any(is.na(predictions)))
  {
    ...
    ggp <- ggplot2::ggplot(train.data, ggplot2::aes(x =!!ggplot2::sym(independent_variable), y =!!ggplot2::sym(dependent_variable))) +
      ...
      ggplot2::geom_smooth(method = "lm", formula = y ~ assoc_pow(10,x), se = TRUE, color = ssEnv$color_palette_darker[2]) + # The fitted line will be an exponential curve
      ggplot2::labs(
...
assoc_pow <- function(base, exponent) base^exponent
