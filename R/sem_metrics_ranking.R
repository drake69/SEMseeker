sem_metrics_ranking <- function (metric,data_frame, column_to_rank ="REBASED"){
  ...
  if(metric %in% the_lower_the_better_markers){
    data_frame$SCORE <- sem_normalize_minimize(data_frame[,column_to_rank])
  }else if(metric %in% the_higher_the_better_markers){
    data_frame$SCORE <- sem_normalize_maximize(data_frame[,column_to_rank])
  }else{
    core_log_event("ERROR: Metric not found")
    stop()
  }
...
# Normalisation helpers used by sem_metrics_ranking():
#   sem_normalize_minimize: maps x to [0,1] where the MAXIMUM of x → 0 and
#     the MINIMUM → 1 (lower-is-better metrics get a higher normalised score).
#   sem_normalize_maximize: maps x to [0,1] where the MINIMUM of x → 0 and
