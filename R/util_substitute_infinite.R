util_substitute_infinite <- function(x) {
  ...
  core_log_event("INFO: ", format(Sys.time(), "%a %b %d %X %Y"), " replacing infinite values with max/min values.")
  ...
  core_log_event("JOURNAL: Replaced ", n_inf_values, " infinite values with max/min values.")
  as.data.frame(x)
}
