#' lesions_get_bulk — multi-sample bp-window LESIONS computation
#'
#' Versione **multi-sample (bulk)** di lesions_get, usata dal path
#' `analyze_population_bulk()` per processare N sample in parallelo via
...
#' @keywords internal
#' @noRd
lesions_get_bulk <- function(mut_df, sample_cols,
                             LESIONS_BP = NULL,
                             bonf_threshold = 0.05) {

  if (!all(c("CHR", "START") %in% colnames(mut_df))) {
    stop("[lesions_get_bulk] mut_df must contain CHR + START columns")
  }
  ...
  if (is.na(window_bp) || window_bp < 0)
    stop("[lesions_get_bulk] LESIONS_BP must be >= 0 (bp), got: ", LESIONS_BP)
