#' Polars-native equivalent of `data_preparation()` for the AI-061
...
#'      through R-side `data_preparation()`.
#'
...
#' AI-061 lazy path.
#'
#' Scope is intentionally narrower than `data_preparation()`:
#'   - Y-side ONLY (transformation_y, degenerate-row filter). The
#'     transformation_x and IV-factor coding from `data_preparation()`
...
#'   Matches the AI-044 filter applied in `data_preparation()` for the
...
#' @keywords internal
#' @noRd
data_preparation_lazy <- function(pivot_lazy,
                                   sample_cols,
  ...
  if (!inherits(pivot_lazy, "polars_lazy_frame")) {
    if (inherits(pivot_lazy, "polars_data_frame")) {
      ...
      stop("data_preparation_lazy: pivot_lazy must be a polars LazyFrame or DataFrame.",
           call. = FALSE)
    }
  }
  if (!is.character(sample_cols) || length(sample_cols) == 0L) {
    stop("data_preparation_lazy: sample_cols must be a non-empty character vector.",
         call. = FALSE)
  }
  ...
  if (!(trans_y %in% supported)) {
    ...
    warning(sprintf(paste0(
      "data_preparation_lazy: transformation_y='%s' is NOT supported by ",
    ...
    log_event("WARNING: ", format(Sys.time(), "%a %b %d %X %Y"),
              " data_preparation_lazy", fam_str, key_str,
  ...
  # `data_preparation()` guards by adding `min(burden_values[burden_values>0])`,
