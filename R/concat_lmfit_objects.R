...
#' @keywords internal
#' @noRd
concat_lmfit_objects <- function(fit_list) {
  if (!is.list(fit_list) || length(fit_list) == 0L) {
    stop("concat_lmfit_objects: fit_list must be a non-empty list.",
         call. = FALSE)
  }
  ...
  if (!inherits(fit_first, "MArrayLM")) {
    stop("concat_lmfit_objects: every element must inherit from MArrayLM.",
         call. = FALSE)
  }
  ...
  for (i in seq.int(2L, length(fit_list))) {
    ...
    if (!isTRUE(all.equal(f$design, fit_first$design,
                            ...
                            tolerance        = 1e-10))) {
      stop("concat_lmfit_objects: chunk ", i,
