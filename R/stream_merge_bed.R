# ---------------------------------------------------------------------------
# stream_merge_bed() — lazy long→wide merge of per-sample bed/bedgraph files
#
...
#' @keywords internal
#' @noRd
stream_merge_bed <- function(bed_files, marker, figure) {

  if (length(bed_files) == 0L)
    stop("stream_merge_bed(): bed_files is empty")
