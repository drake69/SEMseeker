...
sem_analyze_single_sample <- function(values, thresholds, figure, sample_detail) {

  ssEnv <- core_get_session_info()
  core_log_event("DEBUG: ", format(Sys.time(), "%a %b %d %X %Y"),  "sem_analyze_single_sample:", ssEnv$result_folderData)

  mutation_annotated_sorted <- sem_mutations_get(values, figure,thresholds, sample_detail$Sample_ID)
  ...
  core_log_event("DEBUG: ", format(Sys.time(), "%a %b %d %X %Y"),  "sem_analyze_single_sample:",folder_to_save)
  ...
  lesionWeighted <- sem_lesions_get(grouping_column = "CHR", mutation_annotated_sorted = mutation_annotated_sorted)
