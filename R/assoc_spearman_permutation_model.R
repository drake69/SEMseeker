...
#'
assoc_compute_spearman_permutation <- function(sig.formula,df, shuffle = FALSE)
{
  cols <- colnames(df)
...
#'
assoc_spearman_permutation <- function(family_test, sig.formula, tempDataFrame, independent_variable,plot, samples_sql_condition=samples_sql_condition, key)
{
  ...
  statistic_parameter <-  assoc_compute_spearman_permutation(sig.formula=sig.formula, df=tempDataFrame, shuffle = FALSE)
  res$statistic_parameter <- statistic_parameter

  permutation_vector <- replicate(n_permutations_test, assoc_compute_spearman_permutation(sig.formula=sig.formula, df=tempDataFrame, shuffle = TRUE))
  ...
  if ((pvalue < as.numeric(ssEnv$alpha)) && (n_permutations_test < n_permutations))
    permutation_vector <- replicate(n_permutations, assoc_compute_spearman_permutation(sig.formula=sig.formula, df=tempDataFrame, shuffle=TRUE))
  res$r_model <- "assoc_spearman_permutation"
