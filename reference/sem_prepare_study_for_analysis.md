...
Wraps assoc_covariates_model() + independent_variable validation +
factor conversion for dichotomous families + sample_names construction
with complete_cases filtering.

## Usage

``` r
sem_prepare_study_for_analysis(inference_detail, study_summary, family_test)
```
...
- study_summary:
  ...
  data.frame returned by sem_study_summary_get().

- family_test:

  character. Already util_split_and_clean'd.
