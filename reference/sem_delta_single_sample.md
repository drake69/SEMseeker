...
``` r
sem_delta_single_sample(values, thresholds, sample_detail)
```
...
invisibly NULL; HYPER and HYPO delta results are written as bedgraph.gz
files. Only positions present in both `values` and `thresholds` are
processed (inner join on CHR, START, END via
