...
region, `bayes_analysis()` provides the *probabilistic complement*: per-region
...
> The function was previously named `ss_analysis()`. The old name still works
> as a deprecated alias but emits a warning — please migrate.

<hr>

## Cross-study replication

`intra_study_association_replication()` reruns the association on a target study restricted
...
> The function was previously named `association_to_association()`. The old
> name still works as a deprecated alias but emits a warning.
