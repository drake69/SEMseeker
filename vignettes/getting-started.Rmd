...
# Tiny synthetic dataset so downstream chunks can run during vignette build
# without requiring real IDATs / Bioconductor annotation packages.
set.seed(123)
.n_probes  <- 100L
.n_samples <- 6L
beta_matrix <- matrix(
  rbeta(.n_probes * .n_samples, 4, 6),
  nrow = .n_probes, ncol = .n_samples,
  dimnames = list(
    paste0("cg", formatC(seq_len(.n_probes), width = 8L, flag = "0")),
