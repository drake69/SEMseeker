...
#
# We use a subset of `test_master_features` (real EPIC probe IDs around the
# BWS-relevant imprinting DMRs KCNQ1OT1 and H19/IGF2) so the vignette runs in
# a few seconds during CRAN/Bioc vignette build *and* exercises the same
# biology that the package targets.
#
# To run the FULL pipeline on a real GEO dataset (BWS GSE95486, ~30-60 min),
# see the "Subset rationale & full-pipeline runnable" section below.
...
.n_probes  <- nrow(.demo_probes)
.n_samples <- 6L

set.seed(20210713L)
beta_matrix <- matrix(
  rbeta(.n_probes * .n_samples, 8, 2),
  nrow = .n_probes, ncol = .n_samples,
  dimnames = list(
    .demo_probes$PROBE,
    c("REF1","REF2","REF3","CASE1","CASE2","CASE3")
  )
)
# Inject a handful of strong hypo-epimutations in CASE samples on BWS probes
.epi_idx <- sample(.n_probes, 5L)
beta_matrix[.epi_idx, 4:6] <- pmin(beta_matrix[.epi_idx, 4:6] * 0.1, 0.15)
