...
(genomic clusters of co-occurring mutations), and computes per-region
delta metrics — the signed deviation beyond the reference interval
(`DELTAS`), its ratio to the interval width (`DELTAR`), and their
equal-width and quantile ranked variants (`DELTAP`/`DELTARP`,
`DELTAQ`/`DELTARQ`) — that quantify stochastic epigenetic variability
per region.
...
- `*_DELTAS_*.bed` / `*_DELTAR_*.bed` — per-region signed deviation
  (`DELTAS`) and its ratio to the reference interval (`DELTAR`)
