test_that("is_coord_format detects CHR/START columns", {
  df_coord <- data.frame(
    CHR = c("chr1", "chr2"), START = c(1000L, 2000L), END = c(1001L, 2001L),
    s1 = c(0.8, 0.3), s2 = c(0.5, 0.9)
  )
  expect_true(semseeker:::is_coord_format(df_coord))

  df_illumina <- data.frame(s1 = c(0.8, 0.3), s2 = c(0.5, 0.9),
                              row.names = c("cg00000029", "cg00000165"))
  expect_false(semseeker:::is_coord_format(df_illumina))
})

test_that("is_coord_format accepts lowercase and alias column names", {
  expect_true(semseeker:::is_coord_format(data.frame(chr = "1", start = 100, s1 = 0.5)))
  expect_true(semseeker:::is_coord_format(data.frame(chrom = "X", chromStart = 500, s1 = 0.5)))
  expect_true(semseeker:::is_coord_format(data.frame(seqnames = "1", pos = 100, s1 = 0.5)))
})

test_that("coord_to_semseeker converts wide coord df to probe-indexed matrix", {
  df <- data.frame(
  ...
  result <- semseeker:::coord_to_semseeker(df)
...
test_that("coord_to_semseeker strips chr prefix consistently", {
  df <- data.frame(CHR = c("CHR1", "Chr2", "chr3"),
                   START = c(100L, 200L, 300L),
                   s1 = c(0.5, 0.5, 0.5))
  res <- semseeker:::coord_to_semseeker(df)
  expect_equal(rownames(res), c("1_100", "2_200", "3_300"))
})

test_that("coord_to_semseeker works without END column", {
...
test_that("probe_id_to_coord round-trips with coord_to_semseeker", {
  df <- data.frame(
    CHR = c("chr1","chrX","chr22"), START = c(100L, 200L, 300L),
    s1 = c(0.1, 0.2, 0.3)
  )
  probe_ids <- rownames(semseeker:::coord_to_semseeker(df))
  back      <- semseeker:::probe_id_to_coord(probe_ids)
...
test_that("coord_probe_features builds minimal probe_features df", {
  ids <- c("1_10000", "X_543200", "22_900100")
  pf  <- semseeker:::coord_probe_features(ids)
...
test_that("normalize_signal_input passes through Illumina matrix unchanged", {
  df <- data.frame(s1 = c(0.8, 0.3), row.names = c("cg00000029", "cg00000165"))
  result <- semseeker:::normalize_signal_input(df)
  expect_identical(result, df)
})

test_that("normalize_signal_input converts coordinate data frame", {
