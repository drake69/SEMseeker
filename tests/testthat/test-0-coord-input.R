test_that("is_coord_format detects CHR/START columns", {
  ...
  expect_true(is_coord_format(df_coord))

  df_illumina <- data.frame(s1 = c(0.8, 0.3), s2 = c(0.5, 0.9),
                              row.names = c("cg00000029", "cg00000165"))
  expect_false(is_coord_format(df_illumina))
})

test_that("is_coord_format accepts lowercase and alias column names", {
  expect_true(is_coord_format(data.frame(chr = "1", start = 100, s1 = 0.5)))
  expect_true(is_coord_format(data.frame(chrom = "X", chromStart = 500, s1 = 0.5)))
  expect_true(is_coord_format(data.frame(seqnames = "1", pos = 100, s1 = 0.5)))
})
...
test_that("probe_id_to_coord round-trips with coord_to_semseeker", {
  ...
  probe_ids <- rownames(coord_to_semseeker(df))
  back      <- probe_id_to_coord(probe_ids)
...
test_that("normalize_signal_input passes through Illumina matrix unchanged", {
  ...
  result <- normalize_signal_input(df)
...
test_that("normalize_signal_input converts coordinate data frame", {
  ...
  expect_false(is_coord_format(result))
})
