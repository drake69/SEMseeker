...
#
# Tests for normalize_chr() — the single entry point for chromosome name
# normalization between internal (bare: "1", "X") and output (UCSC: "chr1",
# "chrX") conventions.
# =============================================================================

test_that("internal direction strips chr prefix", {
  expect_equal(normalize_chr(c("chr1", "chr22", "chrX"), "internal"),
               c("1", "22", "X"))
})

test_that("internal direction is case-insensitive", {
  expect_equal(normalize_chr(c("Chr1", "CHR2", "CHRX"), "internal"),
               c("1", "2", "X"))
})

test_that("internal direction is idempotent on bare names", {
  expect_equal(normalize_chr(c("1", "X", "22"), "internal"),
               c("1", "X", "22"))
})

test_that("output direction adds chr prefix", {
  expect_equal(normalize_chr(c("1", "22", "X"), "output"),
               c("chr1", "chr22", "chrX"))
})

test_that("output direction is idempotent on prefixed names", {
  expect_equal(normalize_chr(c("chr1", "chrX"), "output"),
               c("chr1", "chrX"))
})

test_that("output direction preserves existing case of chr prefix", {
  expect_equal(normalize_chr(c("Chr1", "CHR2"), "output"),
               c("Chr1", "CHR2"))
})

test_that("handles factor input", {
  f <- factor(c("chr1", "chr2", "chrX"))
  expect_equal(normalize_chr(f, "internal"), c("1", "2", "X"))
})

test_that("handles integer input", {
  expect_equal(normalize_chr(c(1L, 22L), "internal"), c("1", "22"))
  expect_equal(normalize_chr(c(1L, 22L), "output"), c("chr1", "chr22"))
})

test_that("handles empty vector", {
  expect_equal(normalize_chr(character(0), "internal"), character(0))
  expect_equal(normalize_chr(character(0), "output"), character(0))
})

test_that("invalid direction raises error", {
  expect_error(normalize_chr("chr1", "invalid"))
})

test_that("mixed input: some with chr, some without", {
  expect_equal(normalize_chr(c("chr1", "2", "chrX"), "internal"),
               c("1", "2", "X"))
  expect_equal(normalize_chr(c("chr1", "2", "chrX"), "output"),
               c("chr1", "chr2", "chrX"))
})

test_that("always returns character, never numeric", {
  expect_type(normalize_chr(c(1, 22), "internal"), "character")
  expect_type(normalize_chr(c(1, 22), "output"), "character")
  expect_type(normalize_chr("chr1", "internal"), "character")
})

test_that("round-trip: internal → output → internal is identity", {
  bare <- c("1", "22", "X", "Y", "MT")
  expect_equal(normalize_chr(normalize_chr(bare, "output"), "internal"), bare)
})

test_that("round-trip: output → internal → output is identity", {
  prefixed <- c("chr1", "chr22", "chrX", "chrY", "chrMT")
  expect_equal(normalize_chr(normalize_chr(prefixed, "internal"), "output"),
               prefixed)
})
...
# Reading back without normalize_chr("internal") breaks the join.
# ---------------------------------------------------------------------------
test_that("BED round-trip: bare CHR survives write+read cycle", {
  ...
  bed_out <- internal_df
  bed_out$CHR <- normalize_chr(bed_out$CHR, "output")
  ...
  bed_in$CHR <- normalize_chr(bed_in$CHR, "internal")
